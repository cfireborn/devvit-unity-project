...
The game is a WebGL build hosted on **GitHub Pages** (`cfireborn.github.io/compersion2d`). It is not embedded in Reddit or Devvit. The server runs on **Edgegap** with a Cloudflare Tunnel (`compersion.charliefeuerborn.com`) handling WSS for WebGL clients. Do not assume any Reddit/Devvit CSP constraints apply.

---

## Project Context

Full architecture details, file map, known gotchas, and current phase of work live in `AgentDocumentation/`. Read `AgentDocumentation/AGENTS-MOSTRECENT.md` before starting any multiplayer task. The folder also contains `AgentDocumentation/AGENTS.md` (an older copy of this file — prefer this root copy).
