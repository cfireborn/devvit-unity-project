...
The next candidate reduces FishNet physics/network ticks from 60 to 40 Hz and caps only the dedicated-server render loop at 55 FPS. Forty hertz cuts simulation work by one third while keeping cloud snapshots at 20 Hz (`ceil(40 / 20) = 2`) and player input/physics opportunities at 25 ms. Fifty-five server frames leave room for FishNet's 40 simulation ticks without restoring the prior 75-frame sentinel behavior. This is a measured experiment, not a completed performance fix: Editor-host testing uses the unchanged 500 client cap, and acceptance requires a matching Linux build plus sustained populated-world CPU measurements. A 30 Hz candidate was rejected because it would reduce clouds to 15 snapshots per second, raise physics/input intervals to about 33 ms, and compound platformer feel changes.
...
- Production load tests before the 40/55 candidate exposed roughly 77-100% CPU with one populated WebGL client and 98-110% with two on the 0.25-vCPU profile. The candidate therefore requires fresh sustained one-client and two-client measurements; it must not be described as fixed or meeting the requested 20% target until that evidence exists.
...
| `Assets/Levels/SimpleLevel.unity` | Explicit boundary binding, four-unit ladder reach, 40 Hz FishNet tick candidate, and 55 FPS dedicated cap |
