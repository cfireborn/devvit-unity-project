...
The latest Editor spot-check on 2026-09-04 used the Apple iPhone 13 Pro Simulator profile. Admin checkpoint `4/7` opened the authored card at Spike; the single imported polyamory-flag Sprite rendered beneath the optically centered title, and the full title panel stayed below the portrait cutout. Space dismissed the completed card into Spike's first reply line. Rotating the same running session to landscape confirmed Spike's dialogue text stayed inside the painted bubble after the responsive text-anchor fix. The earlier Admin-over-card/close-button check remains valid because this pass did not change sorting or input code. Physical/WebGL touch, an exported desktop WebGL build, host, and pure-client cases remain checklist items below.
...
`CompersionTitleDialogue.presentation` is `CompersionTitleCard`; every other dialogue asset defaults to `Standard`. `DialogueTrigger.ShowDialogue()` creates one scoped completion callback, first asks `GameUIManager.TryShowCompersionTitleCard()` to present it, and falls back to the existing readable dialogue panel when the title prefab or Canvas is unavailable. The fallback prevents a broken prefab reference from consuming Spike's one-shot trigger and stalling the quest. Missing child Sprite assignments do not trigger this fallback; they create an incomplete card and must be caught by prefab/import QA.

`GameUIManager` instantiates the authored `Assets/UI/CompersionTitleCard.prefab` under the ordinary local Canvas. Five transparent type Sprites sit over the runtime backdrop/panel derivatives in `Assets/UI/compersion-title-card/backdrop-kit/`: an aspect-fill atmospheric backdrop, title/wide/compact panels, and the thin continue ornament above its prompt. `Assets/UI/poly flag.png` is one Preserve-Aspect Sprite directly under the title; there is no runtime reconstruction from stripes, a chevron, or a font glyph. `Assets/UI/UI.prefab` stores the required `CompersionTitleCardUI` component reference. A missing prefab/Canvas selects the readable dialogue fallback without changing story state.

The mobile reference layout keeps `TitleBeat` at Y `300`, height `220`, and `120px` total horizontal inset on a `540×960` Canvas. Its top is Y `890`, leaving `70px` for phone cutouts while its bottom remains just above the pronunciation panel. `TitleArtwork` uses anchors `(0.08, 0.50)..(0.92, 0.90)`, a `+20px` X correction for asymmetric transparent PNG padding, and Preserve Aspect. The flag uses `(0.39, 0.06)..(0.61, 0.42)` with Preserve Aspect.
...
The ordinary dialogue panel has a separate wide-screen safeguard. `DialogueBox.png` contains transparent horizontal padding, so `Assets/UI/Dialogue_Panel.prefab` anchors its `Dialogue` TMP object to `0.13..0.87` of the parent width and then applies `20px` left/right margins. This keeps Gray and Spike text inside the painted bubble on both portrait and wide desktop/WebGL canvases. Do not replace those fractional anchors with full-width `0..1` anchors unless the bubble artwork is also recropped.
...
| COMPERSION uses plain dialogue instead of artwork | Missing `presentation: CompersionTitleCard`, invalid `GameUIManager.compersionTitleCardPrefab` component reference, or no Canvas | Verify the dialogue asset flag and that `Assets/UI/UI.prefab` resolves the prefab component field; the plain card is an intentional fail-safe. |
| COMPERSION opens with missing artwork | A child Image on `CompersionTitleCard.prefab` lost its Sprite or an image is not imported as Sprite (2D and UI) | Inspect the Sprite assignments on the authored prefab, including `Assets/UI/poly flag.png`; missing child art does not select the plain-dialogue fallback. |
