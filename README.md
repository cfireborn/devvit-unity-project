...
The implemented story is a local Gray → Spike → Gray delivery loop. An animated, touch/Space-skippable `COMPERSION` title card appears before Spike's reply; the return delivery opens Gray's final dialogue, and completing that dialogue opens a lighthearted developer message with links to the narrative script and studio mailing list. After visiting both, the player can return to free exploration. The fixed-ladder tutorial and delivery-cloud/postbox branch were deliberately removed. The Admin Panel includes an ordered seven-checkpoint Previous/Apply/Next debugger—including the live title-card phase—for exact local story snapshots and Gray/Spike/spawn teleports. See the [story implementation and recovery runbook](Docs/Agents/STORY_THROUGH_SECOND_GOAL.md) for the exact sequence and debugger invariants, and the [Unity Editor workflow](Docs/Agents/UNITY_EDITOR_WORKFLOW.md) before changing scene, dialogue, title artwork, or authored admin UI.
...
Publication is prepared and committed in a second temporary worktree of the website repository. The destructive synchronization never touches the live website checkout before the release is committed, `.nojekyll` is preserved, and the remote `main` SHA is checked again before push. If the website advances elsewhere during the Unity build, the publisher verifies that the remote `compersion` tree is identical and safely builds on top of the newer commit; an actual competing game release still aborts. After a successful push, a clean local website checkout is fast-forwarded; if it changed while Unity was building, it is left untouched.
